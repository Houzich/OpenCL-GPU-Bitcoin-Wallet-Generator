...
#define INT_BITS 64
#define rotate(a,n) ((a << n) | (a >> (INT_BITS - n)))
...
static void memcpy_MY(uchar* dest, const uchar* src, size_t n) {
    for (int i = 0; i < n; i++) {
        dest[i] = src[i];
    }
}
...
static void memset_MY(uchar* str, int c, size_t n) {
    for (int i = 0; i < n; i++) {
        str[i] = c;
    }
}

static void memzero_MY(void* const pnt, const size_t len) {
...
static void memczero_MY(void* s, size_t len, int flag) {
    uchar* p = (uchar*)s;
...
static void secp256k1_ecmult_gen(secp256k1_gej* r, secp256k1_scalar* gn) {
    ...
    memset_MY((uchar*)&adds, 0, sizeof(adds));
...
int secp256k1_ec_pubkey_create(secp256k1_pubkey* pubkey, const uchar* seckey) {
    ...
    memset_MY((uchar*)pubkey, 0, sizeof(*pubkey));
    ...
    memczero_MY(pubkey, sizeof(*pubkey), !ret);
...
void public_from_private(extended_private_key_t* priv, extended_public_key_t* pub) {
    ...
    memcpy_MY((uchar*)&pub->parent_fingerprint, (const uchar*)&priv->parent_fingerprint, 4);
    memcpy_MY((uchar*)&pub->chain_code, (const uchar*)&priv->chain_code, 32);
...
void hardened_private_child_from_private(extended_private_key_t* parent, extended_private_key_t* child, uint hardened_child_number) {
    ...
    memcpy_MY((uchar*)&sk.key, (const uchar*)&hmacsha512_result, 32);
...
{
    memzero_MY(ctx, sizeof(RIPEMD160_CTX));
    ctx->total[0] = 0;
    ctx->total[1] = 0;
...
void ripemd160_Update(RIPEMD160_CTX* ctx, const uchar* input, uint ilen)
{
    ...
    if (left && ilen >= fill)
    {
        memcpy_MY((uchar*)(ctx->buffer + left), input, fill);
    ...
    if (ilen > 0)
    {
        memcpy_MY((uchar*)(ctx->buffer + left), input, ilen);
    }
}
...
void ripemd160_MY(const uchar* msg, uint msg_len, uchar hash[20])
{
    RIPEMD160_CTX ctx;
    ripemd160_Init(&ctx);
...
void hash160(uchar* input, int input_len, char* output) {
    ...
    ripemd160_MY((const uchar*)&sha256_result, 32, (uchar*)output);
}
