...
__kernel void mnemonic_to_seed_1(__global const uchar* mnemonic_phrase, __global uchar* target_seed, __global uchar* buff) {
    const int idx = get_global_id(0);
    //const int idx_l = get_local_id(0);
    ...
    //#define BUFF_SIZE (128 + 128 + 256 + 64 + 8*8 + 120*8)
#define BUFF_SIZE (128 + 128 + 256 + 64)
    ...
    //__global ulong* State = &buff[idx * BUFF_SIZE + 576];
    //__global ulong* W = &buff[idx * BUFF_SIZE + 640];

    //__local ulong State[8];
    //__local ulong W[120];
    ...
    //printf("key_previous_concat_gpu: ");
    //for (int i = 0; i < 256; i++)
    //	printf("0x%02X ", key_previous_concat_gpu[i]);
    //printf("\n");


    sha512_GPU((__global ulong*)key_previous_concat_gpu, 140, (__global ulong*)sha512_result_gpu);
    //printf("****************************************************************\n");
    //printf("sha512_GPU_result 1: ");
    //for (int i = 0; i < 64; i++)
    //	printf("0x%02X ", sha512_result_gpu[i]);
    //printf("\n");
    COPY_PAD_PREVIOUS(opad_key_gpu, sha512_result_gpu, key_previous_concat_gpu);
    //printf("key_previous_concat_gpu 2: ");
    //for (int i = 0; i < 256; i++)
    //	printf("0x%02X ", key_previous_concat_gpu[i]);
    //printf("\n");
    ...
    //printf("ipad_key: ");
    //for (int i = 0; i < 128; i++)
    //	printf("0x%02X ", ipad_key[i]);
    //printf("\n");

    //printf("opad_key: ");
    //for (int i = 0; i < 128; i++)
    //	printf("0x%02X ", opad_key[i]);
    //printf("\n");

    //printf("key_previous_concat: ");
    //for (int i = 0; i < 256; i++)
    //	printf("0x%02X ", key_previous_concat[i]);
    //printf("\n");


    //sha512((ulong*)&key_previous_concat, 140, (ulong*)&sha512_result);
    //printf("****************************************************************\n");
    //printf("sha512_result 1: ");
    //for (int i = 0; i < 64; i++)
    //	printf("0x%02X ", sha512_result[i]);
    //printf("\n");

    //copy_pad_previous((uchar*)&opad_key, (uchar*)&sha512_result, (uchar*)&key_previous_concat);
    //printf("key_previous_concat 2: ");
    //for (int i = 0; i < 256; i++)
    //	printf("0x%02X ", key_previous_concat[i]);
    //printf("\n");
    ...
    //printf("sha512_result 3: ");
    //for (int i = 0; i < 64; i++)
    //	printf("0x%02X ", sha512_result[i]);
    //printf("\n");

    //xor_seed_with_round((char*)&seed, (char*)&sha512_result);
    //printf("seed 4: ");
    //for (int i = 0; i < 64; i++)
...
__kernel void mnemonic_to_seed_2(__global const uchar* mnemonic_phrase, __global uchar* target_seed, __global uchar* buff) {
    const int idx = get_global_id(0);
...
__kernel void mnemonic_to_seed_and_address(__global const uchar* mnemonic_phrase, __global uchar* target_seed, __global uchar* buff) {
    const int idx = get_global_id(0);
...
#define BUFF_SIZE (128 + 128 + 256 + 64)
    ...
    //MY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //new_master_from_seed(network, (uchar *)&seed, &master_private);
//    public_from_private(&master_private, &master_public);

//    uchar serialized_master_public[33];
//    serialized_public_key(&master_public, (uchar *)&serialized_master_public);
...
//    hash160_for_public_key(&target_public_key, (uchar *)&hash160_address);
