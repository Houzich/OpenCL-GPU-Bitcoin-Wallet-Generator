...
void Check_Addressses(void) {
	...
	start_time_forCheck();
	//Get_Derived_BIP44_BIP84_Addressses(seed_buffer, bip44_addresses, bip84_addresses);
...
#pragma omp parallel for num_threads(NUM_THREADS_SQLITE)
	for (int i = 0; i < NUM_ADDRESSES; i++) {
		int num_thread = omp_get_thread_num();
		//printf("num_thread %d\n", num_thread);
		const char* address = (const char*)&bip44_addresses[35 * i];
		std::string table_name = get_table_name_from_char(address[1]);
		int find = db_bip44[num_thread].search_address(table_name, address);
		if (find == 1) {
		...
		//find = db_bip84[num_thread].search_address(table_name, address);
		//if (find == 1) {
...
//	for (int i = 0; i < NUM_ADDRESSES; i++) {
//		int num_thread = omp_get_thread_num();
//		const char* address = (const char*)&bip84_addresses[43 * i];
//		std::string table_name = get_table_name_from_char(address[4]);
//		int find = db_bip84[num_thread].search_address(table_name, address);
//		if (find == 1) {
//			out_idx_bip84.push_back(i);
//		}
//			
...
void Search_Addressses(void) {
	Sqlite3 db_bip44[NUM_THREADS_SQLITE];
	Sqlite3 db_bip84[NUM_THREADS_SQLITE];
#if defined(_WIN32)
	...
	*/
	...
	try
	{
		for (int i = 0; i < NUM_THREADS_SQLITE; i++) {
			db_bip44[i].open("BIP44_Addresses.db");
			db_bip84[i].open("BIP84_Addresses.db");
		}
...
int Test4(void)
{
	...
	//Clear_File();
	cl_ulong number_of_addresses = 50000000;
	...
	for (int i = 0; i < CARDS_NUMBER; i++)
	{
		if (CL_SUCCESS != CreateAndBuildProgramCrypto(&ocl, i, SOURCE_FILE_CODE_BIN))
		{
			goto Error;
		}
	}

	printf("GENERATE %lu ADDRESSES. PACKET %d. WAIT...\n\n", (int)number_of_addresses, (int)(NUM_ADDRESSES * CARDS_NUMBER));
	start_time();
...
#pragma omp parallel for
		...
		for (int i = 0; i < CARDS_NUMBER; i++)
		{
			err = clReleaseKernel(ocl.kernel[i]);
			if (err != CL_SUCCESS)
			{
				LogError("Error: clReleaseKernel returned %s\n", TranslateOpenCLError(err));
				goto Error;
			...
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_MNEMONIC_BITS, i);
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_MNEMONIC, i);
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_SEED, i);
			if (err != CL_SUCCESS)
			{
				goto Error;
			}
...
#pragma omp parallel for
		...
		for (int i = 0; i < CARDS_NUMBER; i++)
		{
			...
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_MNEMONIC, i);
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_SEED, i);
			err |= ReleaseMemObject(&ocl, CRYPTO_BUFF_WORK, i);
			if (err != CL_SUCCESS)
			{
				goto Error;
			}
...
void Get_Derived_BIP44_BIP84_Addressses(uint8_t* seeds, char* bip44_addr, char* bip84_addr) {
	...
	//secp256k1_ctx_XXX = secp256k1_context_create(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
#pragma omp parallel for num_threads(NUM_THREADS_CHECK)
	for (int i = 0; i < NUM_ADDRESSES * CARDS_NUMBER; i++) {
